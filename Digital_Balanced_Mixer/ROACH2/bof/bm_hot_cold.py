...
def hot_cold_roach(save_path,lo_freq,lo_pow, roach, rigol, rs_supply,psg_source, pow0_info, pow1_info, cross_info,cal0_info, 
        cal1_info, synth_info, channels, cal_iters, n_frames,rf_off=False):
...
def hot_cold_analog_direct(save_path,lo_freq,lo_pow,roach, rigol,rs_supply,psg_source, pow0_info,pow1_info,
        ...
        ####################################################################
        print('Turning on LO noise')
        turn_on_LO_noise(rs_supply, sleep_time=1)
        time.sleep(1.5)
        ...
        turn_off_LO_noise(rs_supply, sleep_time=1)
        time.sleep(1.5)
...
if __name__ == '__main__':
    ...
    for lo_freq in lo_freqs:
        for lo_pow in lo_power:
            hot_cold_roach(save_path,lo_freq,lo_pow, roach, rigol, rs_supply,psg_source, pow0_info, pow1_info, cross_info,cal0_info, 
                cal1_info, synth_info, channels, cal_iters, n_frames,rf_off=False)
    ...
    for lo_freq in lo_freqs:
        for lo_pow in lo_power: 
            hot_cold_analog_direct(save_path,lo_freq,lo_pow,roach, rigol,rs_supply,psg_source, pow0_info,pow1_info,
                cal0_info, cal1_info, synth_info, channels)
    turn_off_sequence(rigol, rs_supply, psg_source, sleep_time=3)
    os.system('play -nq -t alsa synth 1 sine 440')
    """
