...
def plot_frb(_fpga, _theta):
    ...
    or_patch = mpatches.Patch(color='orange', label='avg')
    gr_patch = mpatches.Patch(color='green', label='avg+$ \\alpha \\cdot$var')
...
def animate(i):
    ...
    avg0_pow = 10*np.log10(avg0+1)
    avg1_pow = 10*np.log10(avg1+1)
    pow_dat1 = 10*np.log10(dat1+1)
    pow_dat2 = 10*np.log10(dat2+1)

    ani[0].set_data(np.arange(2**10), dat1)#pow_dat1)
    ani[1].set_data(np.arange(2**10), dat2)#pow_dat2)
    ani[2].set_data(np.arange(2**10), avg0)#avg0_pow)
    ani[3].set_data(np.arange(2**10), avg1)#avg3_pow)
    ani[4].set_data(np.arange(2**10), avg0+theta*var0)#avg3_pow)
    ani[5].set_data(np.arange(2**10), avg1+theta*var1)#avg3_pow)
