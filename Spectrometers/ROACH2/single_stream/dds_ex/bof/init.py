...
def plot_specs(_fpga, _freq=[0, 100]):
...
def animate(i):
    #check https://www.mail-archive.com/casper@lists.berkeley.edu/msg07824.html
    ...
    """
    data[0].set_data(freq, 10*np.log10(spec0+1))
    data[1].set_data(freq, 10*np.log10(spec1+1))
    return data

if __name__ == '__main__':
    roach_ip = '192.168.0.40'
    #boffile = 'dds_dft.bof.gz' #this one has cos, -sin in the dds
    boffile = 'dds_dft_2.bof.gz' #this one has cos, sin in the dds
    roach = calan.initialize_roach(roach_ip, boffile=boffile, upload=1)
    roach.write_int('cnt_rst',1)
    fs = 100.
    freq = 25
    pinc = freq/fs
    pinc_fix = int(2.**16*pinc)
    #pinc_fix = int(pinc/2**16)*2**16
    roach.write_int('phase', pinc_fix)
